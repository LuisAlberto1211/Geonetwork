...
class PaginationPrototype {
     ...
     */
    function PaginationPrototype($params = array())
    {
        if (count($params) > 0)
        {
...
//        }
    ...
    function my_link_to_remote($url, $text, $div, $pars=array()){
        ...
        $html = "<a href=\"#\"  onclick=\"new Ajax.Updater('$div','$url',{method: 'post', parameters:{".$this->moreVar.$par."}, evalScripts:true}); return false;\" />$text</a>";
        else:
        $html = "<a href=\"#\"  onclick=\"new Ajax.Updater('$div','$url',{method: 'post', parameters:{".$this->moreVar."}, evalScripts:true}); return false;\" />$text</a>";
        endif;
    return $html;
    }
}
